...
clickbait_output_file = r"../data/clickbait/r1/extracted_clickbait.txt"
...
expanded_file = r"../data/clickbait/r1/expanded_clickbait.txt"
optimized_expanded_file = r"../data/clickbait/r1/optimized_expanded_clickbait.txt"
...
def expand_existing_texts_r2():
    # 1. 定义每个类型的初始样例
    urgency_texts = [
        "快来领取", "速来抢购", "别错过", "限时优惠", "火速领取",
        "仅限今天", "今天最后一天", "抢购倒计时", "快点抢购", "马上抢购"
    ]
    
    reward_texts = [
        "免费领取", "这里有奖励", "免费试用", "免费复活", "赚取积分",
        "免费赠送", "免费体验", "送你红包", "立刻领取", "尽情享受奖励"
    ]
    
    curiosity_texts = [
        "想知道更多吗？", "点击了解", "解锁更多", "点击获取", "立即查看",
        "别错过这次机会", "快速查看详情", "看完就知道", "揭秘真相", "查看更多"
    ]
    
    action_texts = [
        "快点来玩", "点击马上开始", "马上体验", "赶快加入", "立即下载",
        "点击马上体验", "快速进入游戏", "立即开启", "马上参与", "点击开始"
    ]
    
    numeric_texts = [
        "超过10万人选择", "立即领取100元", "只需1元即可体验", "优惠仅限前100名", "限时50%折扣",
        "100%返现", "高达500元奖励", "充值即送100元", "每日签到送积分", "前100名免单"
    ]
    
    emotional_texts = [
        "赚大了！", "好机会不等人", "这简直太划算了", "机会难得，快来抢", "错过就没有了",
        "绝对不容错过", "立刻行动，机会无限", "快来体验，一生一次", "今天不来，永远没有", "今天就是最好的机会"
    ]
    
    # 2. 定义每种类型的Prompt
    urgency_prompt = rf"""
    下面是一些关于"紧迫感"的诱导点击短语：{', '.join(urgency_texts)}。
    请根据这些示例，生成更多类似的诱导点击短语,可使用同义词替换，更新名词，动词，扩充文本，要求共100条，且每条短语不可重复，涵盖各种不同的表达方式，结果返回为一个数组。
    """
    
    reward_prompt = rf"""
    下面是一些关于"奖励/好处"的诱导点击短语：{', '.join(reward_texts)}。
    请根据这些示例，生成更多类似的诱导点击短语，可使用同义词替换，更新名词，动词，扩充文本，要求共100条，要求共100条，且每条短语不可重复，涵盖各种不同的表达方式，结果返回为一个数组。
    """
    
    curiosity_prompt = rf"""
    下面是一些关于"激发好奇心"的诱导点击短语：{', '.join(curiosity_texts)}。
    请根据这些示例，生成更多类似的诱导点击短语，可使用同义词替换，更新名词，动词，扩充文本，要求共100条，要求共100条，且每条短语不可重复，涵盖各种不同的表达方式，结果返回为一个数组。
    """
    
    action_prompt = rf"""
    下面是一些关于"行动驱动"的诱导点击短语：{', '.join(action_texts)}。
    请根据这些示例，生成更多类似的诱导点击短语，可使用同义词替换，更新名词，动词，扩充文本，要求共100条，要求共100条，且每条短语不可重复，涵盖各种不同的表达方式，结果返回为一个数组。
    """
    
    numeric_prompt = rf"""
    下面是一些关于"数字诱惑"的诱导点击短语：{', '.join(numeric_texts)}。
    请根据这些示例，生成更多类似的诱导点击短语，可使用同义词替换，更新名词，动词，扩充文本，要求共100条，要求共100条，且每条短语不可重复，涵盖各种不同的表达方式，结果返回为一个数组。
    """
    
    emotional_prompt = rf"""
    下面是一些关于"情绪化/夸张"的诱导点击短语：{', '.join(emotional_texts)}。
    请根据这些示例，生成更多类似的诱导点击短语，可使用同义词替换，更新名词，动词，扩充文本，要求共100条，要求共100条，且每条短语不可重复，涵盖各种不同的表达方式，结果返回为一个数组。
    """
    
    # 3. 执行请求并获取LLM扩展结果
    def generate_expanded_texts(prompt):
        try:
            chat_response = client.chat.completions.create(
                model="Qwen2.5-32B-Instruct",
                messages=[
    ...
    optimized_expanded_file = r"../data/clickbait/r2_clickbait.txt"

    # 扩展每种类型的诱导文本并保存到文件
    def save_expanded_texts(initial_texts, expanded_texts, category_name):
        all_texts = initial_texts + expanded_texts.split('\n')
        with open(optimized_expanded_file, 'a', encoding='utf-8') as out_file:
    ...
    # 扩展并保存每个类型的文本
    urgency_expanded = generate_expanded_texts(urgency_prompt)
    reward_expanded = generate_expanded_texts(reward_prompt)
    curiosity_expanded = generate_expanded_texts(curiosity_prompt)
    action_expanded = generate_expanded_texts(action_prompt)
    numeric_expanded = generate_expanded_texts(numeric_prompt)
    emotional_expanded = generate_expanded_texts(emotional_prompt)
    
    save_expanded_texts(urgency_texts, urgency_expanded, "紧迫感")
    save_expanded_texts(reward_texts, reward_expanded, "奖励/好处")
    save_expanded_texts(curiosity_texts, curiosity_expanded, "激发好奇心")
    save_expanded_texts(action_texts, action_expanded, "行动驱动")
    save_expanded_texts(numeric_texts, numeric_expanded, "数字诱惑")
    save_expanded_texts(emotional_texts, emotional_expanded, "情绪化/夸张")
